...
### Planned features
- **Nat traversal** This currently works in a relay mode, all traffic, like with the traditional VPN, goes via the VPN server. But it is sometimes possible to establish a direct connection between devices, even behind their own routers that hide device's IP addresses (NAT). We will likely use [Hypercore's Hyperswarm](https://github.com/hyperswarm/hyperswarm) for this.
